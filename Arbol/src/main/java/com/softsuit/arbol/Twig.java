 ...
 *
 * @param <T> Any Object of your choice used to describe or store information
 *            about this twig.
 */
public class Twig<T> implements Twigable<T> {
	...
	public void addRamification(final Twig<T> twig) {
		...
		final int nextIdIndex = ((this.id.length() + 1) > twig.getId().length()) ? twig.getId().length()
				: (this.id.length() + 1);
	...
	public Optional<Twig<T>> traceBackwords(String id) {
		...
		// may not have a parent, if it is outgoing from trunk, 
		// in this case use traceFromTrunk if you still want to search
		return Optional.empty();
	}
